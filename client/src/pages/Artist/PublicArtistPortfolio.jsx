  ...
  const fetchArtistData = async () => {
    try {
      ...
      const profileResponse = await axios.get(
        `${API_URL}/api/buyer/artists/${artistId}/profile`,
        {
          headers: {
      ...
      const getImageUrl = (relativePath) => {
        ...
        // Serve images directly from client public folder (no backend request)
      ...
      const formattedArtistProfile = {
        ...
        joinDate: artistData.joinDate
          ? new Date(artistData.joinDate).toLocaleDateString("en-US", {
              month: "long",
              year: "numeric",
            })
          : "Recently",
        ...
        avatar:
          getImageUrl(artistData.avatarUrl) ||
          "/uploads/profiles/default-avatar.svg",
        coverImage:
          getImageUrl(artistData.coverImageUrl) || "/default-cover.jpg",
      ...
      try {
        const postsResponse = await axios.get(
          `${API_URL}/api/posts/artist/${artistId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );

        // Process and format the posts data to match frontend expectations
        const formattedPosts = (postsResponse.data || []).map((post) => ({
          id: post.post_id,
          caption: post.caption,
          // Use the first image if multiple images exist, or handle multiple images
          image:
            post.images && post.images.length > 0
              ? post.images[0].startsWith("/uploads/")
                ? post.images[0] // Keep the full path for posts: /uploads/filename.webp
                : post.images[0].startsWith("/")
                ? post.images[0].substring(1)
                : post.images[0]
              : null,
          images: post.images || [], // Keep all images for potential carousel/gallery
          location: post.location,
          timestamp: post.created_at,
          likes: post.likes || 0,
          comments: post.comments || 0, // Add fallback for comments
        }));

        console.log("Formatted portfolio posts:", formattedPosts);
        setPortfolioPosts(formattedPosts);
      } catch (postsError) {
      ...
      try {
        const artworksResponse = await axios.get(
          `${API_URL}/api/artworks/artist/${artistId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
      ...
      try {
        const exhibitionsResponse = await axios.get(
          `${API_URL}/api/exhibitions/artist/${artistId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
      ...
      try {
        const achievementsResponse = await axios.get(
          `${API_URL}/api/achievements/artist/${artistId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
      ...
      if (error.response?.status === 404) {
        ...
        setError(
          "Authentication required. Please login to view artist profiles."
        );
      } else if (
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                    ...
                    <div>
                      <div className="text-2xl font-bold text-[#7f5539]">
                        {(artistProfile?.stats?.followers || 0) +
                          (isFollowing ? 1 : 0)}
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-[#7f5539]/70">
                    {artistProfile?.priceRange || "Contact for pricing"}
                  </p>
          ...
          <div className="border-b border-[#fdf9f4]/50">
            <nav className="flex space-x-8 px-6">
              {[
                ...
                {
                  id: "artworks",
                  label: "Artworks for Sale",
                  count: artworks.filter((a) => a.status === "Available")
                    .length,
                },
                {
                  id: "exhibitions",
                ...
                <div className="bg-white rounded-lg shadow-sm border border-[#fdf9f4]/20 p-6">
                  ...
                  <div className="space-y-3">
                    {achievements && achievements.length > 0 ? (
                      ...
                      <p className="text-[#7f5539]/60 text-sm text-center py-4">
                        No achievements yet
                      </p>
                    )}
                  </div>
                    ...
                    <div className="flex items-center justify-between p-5">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-[#7f5539]/60">
                            {safeFormatDistanceToNow(post.timestamp)}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-[#fdf9f4]/20 p-6">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-[#7f5539]/70">
                        Joined {artistProfile?.joinDate || "Recently"}
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Palette className="h-4 w-4 text-[#7f5539]/60" />
                      <span className="text-[#7f5539]/70">
                        {(artistProfile?.specialties || []).join(", ") ||
                          "Various specialties"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {artworks
                      ...
                      .map((artwork) => (
                        <div
                          key={artwork.id}
                          className="flex items-center space-x-3 cursor-pointer hover:bg-[#fdf9f4]/30 p-2 rounded-lg transition-colors"
        ...
        {activeTab === "exhibitions" && (
          <div className="bg-white rounded-lg shadow-sm p-6">
            <h3 className="text-lg font-semibold text-[#7f5539] mb-6">
              Exhibitions
            </h3>
